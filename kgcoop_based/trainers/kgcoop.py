...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.n_cls = n_cls + int(cfg.empty_cls_prompt)
        self.n_ctx = n_ctx
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model, device):
        ...
        from .coop import load_clip_to_cpu as _load
        from .zsclip import CUSTOM_TEMPLATES

        _clip_model = _load(cfg)
        ...
        with torch.no_grad():
            text_features = _clip_model.encode_text(prompts)
            text_features = text_features / text_features.norm(dim=-1, keepdim=True)

        self.text_features = text_features
        self.orig_clip_image_encoder = _clip_model.visual
    ...
    def forward(self, image):
        ...
        if self.cfg.empty_cls_prompt:
            score = cos(text_features[:-1,:], text_features_old)
        else:
