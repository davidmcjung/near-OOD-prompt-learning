...
class SimpleTrainer(TrainerBase):
    ...
    def build_data_loader(self):
        ...
        self.n_train_samples = self.train_loader_x.dataset.__len__()
    ...
    def test(self, split=None):
        ...
        if self.cfg.empty_cls_prompt:
            ...
            for batch in tqdm(train_data_loader): 
                ...
                with torch.no_grad():
                    ...
                    if 'LoCoOp' in self.cfg.TRAINER.NAME:
        ...
        if self.cfg.DATASET.SUBSAMPLE_CLASSES == 'base' and not self.cfg.no_eval_near_ood:
            print('Near-OOD Eval')
            near_ood_outputs = []
            ...
            for ood_method in self.cfg.ood_method:
                ...
                for output in tqdm(in_outputs, desc='ID '+ood_method):
                    if 'LoCoOp' in self.cfg.TRAINER.NAME:
                        ...
                        score = self.ood_score(global_logits=output, local_logits=local_output, method=ood_method, coeff=coeff, empty_cls=self.cfg.empty_cls_prompt)
                    else:
                        output = output[:,:-1]-coeff*output[:,-1:] if self.cfg.empty_cls_prompt and not 'mcm' in ood_method else output
